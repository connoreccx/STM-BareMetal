...
#define DMA1EN			(1U<<21)
#define CHSEL4			(1U<<27)
#define MEM_INC_EN		(1U<<10)
#define DMA_DIR_MemPer	(1U<<6)
#define DMA_CR_TCIE		(1U<<4)
#define CR_EN			(1U<<0)
#define UART3_CR3_DMAT	(1U<<7)
...
void dma1_stream6_init(uint32_t src, uint32_t dst, uint32_t len){
	...
	// Disable DMA1 Stream 6 (Good Practice, make sure to turn it back on)
	DMA1_Stream6->CR &= ~CR_EN;

	// Wait
	while(DMA1_Stream6->CR & CR_EN){}

	// Clear all interrupt flags of stream 6 (Find in DMA1_HIFCR in reference manual)
	DMA1->HIFCR |= (1U<<16);
	DMA1->HIFCR |= (1U<<18);
	DMA1->HIFCR |= (1U<<19);
	DMA1->HIFCR |= (1U<<20);
	...
	// Set destination buffer (Peripheral Address Register = PAR)
	DMA1_Stream6->PAR = dst;

	// Set memory source buffer
	DMA1_Stream6->M0AR = src;

	// Set length
	DMA1_Stream6->NDTR = len;

	// Select stream6 and channel 4 (in ref manual dma1 request mapping)
	DMA1_Stream6->CR = CHSEL4;

	// Enable Memeory Increment
	DMA1_Stream6->CR |= MEM_INC_EN;

	// Configure transfer direction (memory to peripherals)
	DMA1_Stream6->CR |= DMA_DIR_MemPer;
	...
	USART2->CR3 |= UART3_CR3_DMAT;

	// DMA1 Interrupt NVIC_Enable
